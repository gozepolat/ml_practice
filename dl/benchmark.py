...
def cross_validate(nb_class, X, y, nb_epoch, task, labels, n_folds=10, evaluate=False, max_words=100,
    ...
    for i, (train_ix, test_ix) in enumerate(skf):
        ...
        pos_model = models.construct_cnn_lstm(stateful=stateful, convolutional=convolutional, nb_class=nb_class,
                                              max_words=max_words)
        # pretrained_embedding=pre_model.layers[0])
        X_train, X_test = X[train_ix], X[test_ix]
        y_train, y_test = y[train_ix], y[test_ix]
        models.train_model(pos_model, X_train, preprocessing.label_binarize(y_train, classes=labels),
                           nb_epoch=nb_epoch,
        ...
        y_test_pred = remap(y_test_pred, labels)
        ...
        print(", ".join(labels))
        for i, label in enumerate(["precision", "recall", "fscore", "support"]):
